...
async function loadMediaFile(file, workstationOptions = {}) {
  if (!file) return;
  ...
  const url = workstationOptions.url || URL.createObjectURL(file);
  ...
  if (waveformEditor && !preserveProjectWaveform && !workstationOptions.url) {
    try {
      DATA.spectral = null;
